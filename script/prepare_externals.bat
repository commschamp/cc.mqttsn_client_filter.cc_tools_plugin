...
rem CC_MQTTSN_GENERATED_REPO - (Optional) Repository of the cc.mqttsn.generated
rem CC_MQTTSN_GENERATED_TAG - (Optional) Tag of the cc.mqttsn.generated
rem CC_MQTTSN_LIBS_REPO - (Optional) Repository of the cc.mqttsn.libs
rem CC_MQTTSN_LIBS_TAG - (Optional) Tag of the cc.mqttsn.libs
...
if [%CC_MQTTSN_GENERATED_REPO%] == [] set CC_MQTTSN_GENERATED_REPO="https://github.com/commschamp/cc.mqttsn.generated.git"

if [%CC_MQTTSN_GENERATED_TAG%] == [] set CC_MQTTSN_GENERATED_TAG="master"

if [%CC_MQTTSN_LIBS_REPO%] == [] set CC_MQTTSN_LIBS_REPO="https://github.com/commschamp/cc.mqttsn.libs.git"

if [%CC_MQTTSN_LIBS_TAG%] == [] set CC_MQTTSN_LIBS_TAG="master"
...
set CC_MQTTSN_GENERATED_SRC_DIR=%EXTERNALS_DIR%/cc.mqttsn.generated
set CC_MQTTSN_GENERATED_BUILD_DIR=%BUILD_DIR%/externals/cc.mqttsn.generated/build
set CC_MQTTSN_GENERATED_INSTALL_DIR=%CC_MQTTSN_GENERATED_BUILD_DIR%/install
if NOT [%COMMON_INSTALL_DIR%] == [] set CC_MQTTSN_GENERATED_INSTALL_DIR=%COMMON_INSTALL_DIR%

set CC_MQTTSN_LIBS_SRC_DIR=%EXTERNALS_DIR%/cc.mqttsn.libs
set CC_MQTTSN_LIBS_BUILD_DIR=%BUILD_DIR%/externals/cc.mqttsn.libs/build
set CC_MQTTSN_LIBS_INSTALL_DIR=%CC_MQTTSN_LIBS_BUILD_DIR%/install
if NOT [%COMMON_INSTALL_DIR%] == [] set CC_MQTTSN_LIBS_INSTALL_DIR=%COMMON_INSTALL_DIR%
...
if exist %CC_MQTTSN_GENERATED_SRC_DIR%/.git (
    echo "Updating cc.mqttsn.generated..."
    cd %CC_MQTTSN_GENERATED_SRC_DIR%
    git fetch --all
    git checkout .
    git checkout %CC_MQTTSN_GENERATED_TAG%
    git pull --all
) else (
    echo "Cloning cc.mqttsn.generated ..."
    git clone -b %CC_MQTTSN_GENERATED_TAG% %CC_MQTTSN_GENERATED_REPO% %CC_MQTTSN_GENERATED_SRC_DIR%
    if %errorlevel% neq 0 exit /b %errorlevel%
)

echo "Building cc.mqttsn.generated ..."
mkdir "%CC_MQTTSN_GENERATED_BUILD_DIR%"
cd %CC_MQTTSN_GENERATED_BUILD_DIR%
cmake %GENERATOR_PARAM% %PLATFORM_PARAM% -S %CC_MQTTSN_GENERATED_SRC_DIR% -B %CC_MQTTSN_GENERATED_BUILD_DIR% -DCMAKE_INSTALL_PREFIX=%CC_MQTTSN_GENERATED_INSTALL_DIR% -DCMAKE_BUILD_TYPE=%COMMON_BUILD_TYPE% -DOPT_REQUIRE_COMMS_LIB=OFF -DCMAKE_PREFIX_PATH=%COMMS_INSTALL_DIR% -DCMAKE_CXX_STANDARD=%COMMON_CXX_STANDARD%
if %errorlevel% neq 0 exit /b %errorlevel%
cmake --build %CC_MQTTSN_GENERATED_BUILD_DIR% --config %COMMON_BUILD_TYPE% --target install
if %errorlevel% neq 0 exit /b %errorlevel%

rem ----------------------------------------------------

if exist %CC_MQTTSN_LIBS_SRC_DIR%/.git (
    echo "Updating cc.mqttsn.libs..."
    cd %CC_MQTTSN_LIBS_SRC_DIR%
    git fetch --all
    git checkout .
    git checkout %CC_MQTTSN_LIBSD_TAG%
    git pull --all
) else (
    echo "Cloning cc.mqttsn.libs ..."
    git clone -b %CC_MQTTSN_LIBS_TAG% %CC_MQTTSN_LIBS_REPO% %CC_MQTTSN_LIBS_SRC_DIR%
    if %errorlevel% neq 0 exit /b %errorlevel%
)
    
echo "Building cc.mqttsn.libs ..."
mkdir "%CC_MQTTSN_LIBS_BUILD_DIR%"
cd %CC_MQTTSN_LIBS_BUILD_DIR%
cmake %GENERATOR_PARAM% %PLATFORM_PARAM% -S %CC_MQTTSN_LIBS_SRC_DIR% -B %CC_MQTTSN_LIBS_BUILD_DIR% -DCMAKE_INSTALL_PREFIX=%CC_MQTTSN_LIBS_INSTALL_DIR% -DCMAKE_BUILD_TYPE=%COMMON_BUILD_TYPE% -DCMAKE_PREFIX_PATH=%COMMS_INSTALL_DIR%;%CC_MQTTSN_GENERATED_INSTALL_DIR% -DCMAKE_CXX_STANDARD=%COMMON_CXX_STANDARD% -DCC_MQTTSN_CLIENT_LIB_FORCE_PIC=ON -DCC_MQTTSN_CLIENT_APPS=OFF -DCC_MQTTSN_GATEWAY_LIB=OFF
if %errorlevel% neq 0 exit /b %errorlevel%
cmake --build %CC_MQTTSN_LIBS_BUILD_DIR% --config %COMMON_BUILD_TYPE% --target install
if %errorlevel% neq 0 exit /b %errorlevel%
